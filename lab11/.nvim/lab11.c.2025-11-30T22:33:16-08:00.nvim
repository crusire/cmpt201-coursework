...
#define MAX_FILE_SIZE 512
  ...
  // set up verification context with pubkey
  if (EVP_DigestVerifyInit(mdctx, NULL, EVP_sha256(), NULL, pubkey) != 1) {
    handle_error("Error initializing key via pubkey.");
    EVP_MD_CTX_free(mdctx);
    return 0;
  }

  // load message into verification context
  if (EVP_DigestVerifyUpdate(mdctx, message, strlen((char *)message)) != 1) {
    handle_error("Error loading message into verification context");
    EVP_MD_CTX_free(mdctx);
    return 0;
  }

  // verify message using signature
  if (EVP_DigestVerifyFinal(mdctx, signature, strlen((char *)signature)) == 1) {
    EVP_MD_CTX_free(mdctx);
    return 1;
  } else {
    EVP_MD_CTX_free(mdctx);
