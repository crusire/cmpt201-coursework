...
5 strings: "Hello", "Apple", "Car", "Green", "Dog"
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
run_acceptor accepts a struct with a run flag (bool that tells the thread
whether it should keep running or not, a reference to the list where the
messages are stored, and a socket. since it is a pointer to the struct this
info can view the global message list and modify it appropriately
2. How are received messages stored?
a global linked list protected by a mutex.
3. What does `main()` do with the received messages?
- wait until enough messages have been recieved
- stop the acceptor thread with pthread_join
- check the appropriate amount of messages have been recieved
- runs collect_all
- verifies collect_all worked
- clean up the mutex and return
4. How are threads used in this sample code?
the main thread does everything as stated above in question 3. What's key is
that main() starts the acceptor thread which runs in a loop, recieving
messages from clients and storing them into the linked list. the main thread
then waits for the acceptor thread to finish and collects all the messages
*/
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
